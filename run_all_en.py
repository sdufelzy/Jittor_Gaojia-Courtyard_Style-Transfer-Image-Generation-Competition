...
with jt.no_grad():
    for tempid in tqdm.tqdm(range(0, max_num)):
        ...
        for id, prompt in prompts.items():
            ...
            #image = pipe(f"a "+prompt+f" in the style of style_{taskid}, depicted " +cls[int(taskid)-int('0')], num_inference_steps=25, width=512, height=512).images[0]

            image = pipe(f"a " + prompt ,
                         num_inference_steps=25, width=512, height=512).images[0]
            os.makedirs(f"./output/{taskid}/", exist_ok=True)
            image.save(path+f"/{taskid}/images/{prompt}_null.png")
