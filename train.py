...
class DreamBoothDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        a=self.instance_images_path[index % self.num_instance_images]
        a=str(a).split("/")[-1].split(".")[0]
        #example["pro"] =
        #print("encoder")
        #print(self.encoder_hidden_states)
        if self.encoder_hidden_states is not None:
            example["instance_prompt_ids"] = self.encoder_hidden_states
        else:
            b=self.instance_prompt
            ...
            #print(b[-1])
            #print(b[-2])
            c=10*(int(b[-2])-int('0'))+int(b[-1])-int('0')
            #An apple in the style of style_0, depicted in a folding art style.
            #print(c)
            #print(10*(int(b[-2])-int('0')))
            #print(self.instance_prompt)
            #"a "+prompt + f" style_{taskid} in style_i background",
            #
            if(a.split("_")[-1]=='null'):
                self.instance_prompt="a "+a
            else:
                self.instance_prompt="a "+a+" in the style of "+b+ ", depicted "+ cls[c]

            #print(self.instance_prompt)
            text_inputs = tokenize_prompt(
                self.tokenizer, self.instance_prompt, tokenizer_max_length=self.tokenizer_max_length
            )
            self.instance_prompt=b
...
def collate_fn(examples, with_prior_preservation=False):
    ...
    #classes=[example["pro"] for example in examples]
...
def main(args):
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            ...
            #print(nn.mse_loss(jt.histc(target, bins=128),jt.histc(model_pred, bins=128)))
            #print(model_pred[0].shape)
            #ssim1 = friqa.ssim(jt.array(jt.reshape(model_pred[0][0],(64,64))), jt.array(jt.reshape(target[0][0],(64,64))))
            #print(score)
            #print(ssim1)
            loss = nn.mse_loss(model_pred, target)+0.000001*nn.mse_loss(jt.histc(target, bins=128),jt.histc(model_pred, bins=128))+0.5*nn.l1_loss(model_pred,target)##
            loss.backward()
